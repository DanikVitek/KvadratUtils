...
public class ManagePermissionsCommand implements CommandExecutor {
    ...
    private static void setPlayerHeads(Player player, Menu managerMenu) {
        List<ItemStack> pageHeads = PageUtil.getPageItems(getPlayerHeads(player), pages.get(player.getUniqueId()), 45);
    ...
    private static void setPageControls(Player player, Menu managerMenu) {
        ...
        managerMenu.setButton(53, new Button(ControlButtons.ARROW_RIGHT.getItemStack()) {
            ...
            public void onClick(Menu menu, InventoryClickEvent event) {
                event.setCancelled(true);
                pages.put(player.getUniqueId(), (pages.get(player.getUniqueId()) + 1) % PageUtil.getMaxPages(getPlayerHeads(player), 45));
                redrawMenu(player, managerMenu, true);
            }
        });
    }
        ...
        return Bukkit.getOnlinePlayers().stream()
                .map(p ->
                        new ItemBuilder(Material.PLAYER_HEAD)
                                ...
                                .build())
                .filter(i -> {
                    Player subjectPlayer = Bukkit.getPlayer(Objects.requireNonNull(i.getItemMeta()).getDisplayName());
                    return subjectPlayer != null && (!subjectPlayer.hasPermission("kvadratutils.moderator") || player.hasPermission("kvadratutils.moderator"));
                }).collect(Collectors.toList());
    }
}